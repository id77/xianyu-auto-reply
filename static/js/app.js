...
function showSection(sectionName) {
  ...
  menuLinks.forEach((link) => {
    if (
      link.onclick &&
      link.onclick.toString().includes(`showSection('${sectionName}')`)
    ) {
  ...
  switch (sectionName) {
    ...
    case 'logs': // 【日志管理菜单】
      ...
      setTimeout(() => {
        // 检查是否在正确的页面并且元素存在
        const systemLogContainer =
          document.getElementById('systemLogContainer');
  ...
  try {
    ...
    if (cookiesResponse.ok) {
      ...
      accountsWithKeywords.forEach((account) => {
        ...
        const isEnabled =
          account.enabled === undefined ? true : account.enabled;
...
function updateDashboardAccountsList(accounts) {
  ...
  accounts.forEach((account) => {
    ...
    row.innerHTML = `
        <td>
        <strong class="text-primary ${!isEnabled ? 'text-muted' : ''}">${
      account.id
    }</strong>
        ${
          !isEnabled
            ? '<i class="bi bi-pause-circle-fill text-danger ms-1" title="已禁用"></i>'
            : ''
        }
        </td>
        <td>
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (accountsWithKeywords.length === 0) {
        select.innerHTML =
          '<option value="">❌ 暂无账号，请先添加账号</option>';
        return;
      }
      ...
      const enabledAccounts = accountsWithKeywords.filter((account) => {
        ...
        console.log(
          `账号 ${account.id} 过滤状态: enabled=${account.enabled}, 判断为启用=${enabled}`
        ); // 调试信息
        return enabled;
      });
...
async function refreshKeywordsList() {
  ...
  try {
    const response = await fetch(
      `${apiBase}/keywords-with-item-id/${currentCookieId}`,
      {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
  ...
  try {
    ...
    if (accountResponse.ok) {
      ...
      accountStatus = currentAccount
        ? currentAccount.enabled === undefined
          ? true
          : currentAccount.enabled
        : true;
      console.log(
        `加载关键词时账号 ${accountId} 状态: enabled=${currentAccount?.enabled}, accountStatus=${accountStatus}`
      ); // 调试信息
    }

    const response = await fetch(
      `${apiBase}/keywords-with-item-id/${accountId}`,
      {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
...
async function addKeyword() {
  ...
  try {
    ...
    let textKeywords = currentKeywords.filter(
      (item) => (item.type || 'text') === 'text'
    );
    ...
    if (isEditMode && typeof window.editingIndex !== 'undefined') {
      ...
      const originalKeyword =
        keywordsData[currentCookieId][window.editingIndex];
      const textIndex = textKeywords.findIndex(
        (item) =>
    ...
    if (isEditMode && typeof window.editingIndex !== 'undefined') {
      ...
      allKeywords = allKeywords.filter(
        (item, index) => index !== window.editingIndex
      );
    }
    ...
    if (existingKeyword) {
      ...
      showToast(
        `关键词 "${keyword}" ${itemIdText} 已存在（${typeText}关键词），请使用其他关键词或商品ID`,
        'warning'
      );
      toggleLoading(false);
      return;
    ...
    if (response.ok) {
      ...
      try {
        ...
        // 检查是否是重复关键词的错误
        if (
          errorMessage.includes('关键词已存在') ||
          errorMessage.includes('关键词重复') ||
    ...
    keywordItem.innerHTML = `
        ...
        <div class="keyword-actions">
            <button class="action-btn edit-btn ${
              isImageType ? 'edit-btn-disabled' : ''
            }" onclick="${
      isImageType ? 'editImageKeyword' : 'editKeyword'
    }(${index})" title="${isImageType ? '图片关键词不支持编辑' : '编辑'}">
...
async function loadCookies() {
  try {
    ...
    const accountsWithKeywords = await Promise.all(
      cookieDetails.map(async (cookie) => {
        try {
          ...
          // 获取默认回复设置
          const defaultReplyResponse = await fetch(
            `${apiBase}/default-replies/${cookie.id}`,
            {
              headers: { Authorization: `Bearer ${authToken}` },
            }
          );
          ...
          const aiReplyResponse = await fetch(
            `${apiBase}/ai-reply-settings/${cookie.id}`,
            {
              headers: { Authorization: `Bearer ${authToken}` },
            }
          );
    ...
    accountsWithKeywords.forEach((cookie) => {
      ...
      console.log(
        `账号 ${cookie.id} 状态: enabled=${cookie.enabled}, isEnabled=${isEnabled}`
      ); // 调试信息
      ...
      // 自动确认发货状态（默认开启）
      const autoConfirm =
        cookie.auto_confirm === undefined ? true : cookie.auto_confirm;
      ...
      tr.innerHTML = `
        ...
        <td class="align-middle">
            <span class="badge ${
              cookie.keywordCount > 0 ? 'bg-success' : 'bg-secondary'
            }">
        ...
        <td class="align-middle">
            ...
            <label class="status-toggle" title="${
              isEnabled ? '点击禁用' : '点击启用'
            }">
                <input type="checkbox" ${
                  isEnabled ? 'checked' : ''
                } onchange="toggleAccountStatus('${cookie.id}', this.checked)">
                <span class="status-slider"></span>
            </label>
            <span class="status-badge ${
              isEnabled ? 'enabled' : 'disabled'
            }" title="${isEnabled ? '账号已启用' : '账号已禁用'}">
                <i class="bi bi-${
                  isEnabled ? 'check-circle-fill' : 'x-circle-fill'
                }"></i>
            </span>
            </div>
        </td>
        ...
        <td class="align-middle">
            <div class="d-flex align-items-center gap-2">
            <label class="status-toggle" title="${
              autoConfirm ? '点击关闭自动确认发货' : '点击开启自动确认发货'
            }">
                <input type="checkbox" ${
                  autoConfirm ? 'checked' : ''
                } onchange="toggleAutoConfirm('${cookie.id}', this.checked)">
                <span class="status-slider"></span>
            </label>
            <span class="status-badge ${
              autoConfirm ? 'enabled' : 'disabled'
            }" title="${
        autoConfirm ? '自动确认发货已开启' : '自动确认发货已关闭'
      }">
        ...
        <td class="align-middle">
            <div class="remark-cell" data-cookie-id="${cookie.id}">
                <span class="remark-display" onclick="editRemark('${
                  cookie.id
                }', '${(cookie.remark || '').replace(
        /'/g,
        '&#39;'
      )}')" title="点击编辑备注" style="cursor: pointer; color: #6c757d; font-size: 0.875rem;">
                    ${
                      cookie.remark ||
                      '<i class="bi bi-plus-circle text-muted"></i> 添加备注'
                    }
                </span>
            </div>
        </td>
        <td class="align-middle">
            <div class="pause-duration-cell" data-cookie-id="${cookie.id}">
                <span class="pause-duration-display" onclick="editPauseDuration('${
                  cookie.id
                }', ${
        cookie.pause_duration !== undefined ? cookie.pause_duration : 10
      })" title="点击编辑暂停时间" style="cursor: pointer; color: #6c757d; font-size: 0.875rem;">
                    <i class="bi bi-clock me-1"></i>${
                      cookie.pause_duration === 0
                        ? '不暂停'
        ...
        <td class="align-middle">
            ...
            <button class="btn btn-sm btn-outline-primary" onclick="editCookieInline('${
              cookie.id
            }', '${cookie.value}')" title="修改Cookie" ${
        !isEnabled ? 'disabled' : ''
      }>
                <i class="bi bi-pencil"></i>
            </button>
            <button class="btn btn-sm btn-outline-success" onclick="goToAutoReply('${
              cookie.id
            }')" title="${
        isEnabled ? '设置自动回复' : '配置关键词 (账号已禁用)'
      }">
                <i class="bi bi-arrow-right-circle"></i>
            </button>
            <button class="btn btn-sm btn-outline-warning" onclick="configAIReply('${
              cookie.id
            }')" title="配置AI回复" ${!isEnabled ? 'disabled' : ''}>
                <i class="bi bi-robot"></i>
            </button>
            <button class="btn btn-sm btn-outline-info" onclick="copyCookie('${
              cookie.id
            }', '${cookie.value}')" title="复制Cookie">
                <i class="bi bi-clipboard"></i>
            </button>
            <button class="btn btn-sm btn-outline-danger" onclick="delCookie('${
              cookie.id
            }')" title="删除账号">
                <i class="bi bi-trash"></i>
            </button>
...
async function showCooldownStatus(cookieId) {
  ...
  try {
    const response = await fetch(
      `${apiBase}/qr-login/cooldown-status/${cookieId}`,
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${authToken}`,
...
async function resetCooldownTime(cookieId) {
  ...
  try {
    const response = await fetch(
      `${apiBase}/qr-login/reset-cooldown/${cookieId}`,
      {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${authToken}`,
...
function openAccountEditModal(accountData) {
  ...
  document.getElementById('editAccountShowBrowser').checked =
    accountData.show_browser || false;
...
async function toggleAccountStatus(accountId, enabled) {
  try {
    ...
    if (response.ok) {
      ...
      showToast(
        `账号 "${accountId}" 已${enabled ? '启用' : '禁用'} (前端模拟)`,
    ...
    // 后端不支持时的降级处理
    showToast(
      `账号 "${accountId}" 已${enabled ? '启用' : '禁用'} (本地模拟)`,
      enabled ? 'success' : 'warning'
    );
    updateAccountRowStatus(accountId, enabled);
...
function updateAccountRowStatus(accountId, enabled) {
  ...
  const actionButtons = row.querySelectorAll(
    '.btn-group .btn:not(.btn-outline-info):not(.btn-outline-danger)'
  );
...
async function toggleAutoConfirm(accountId, enabled) {
  try {
    ...
    const response = await fetch(
      `${apiBase}/cookies/${accountId}/auto-confirm`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (response.ok) {
      ...
      const toggle = document.querySelector(
        `input[onchange*="toggleAutoConfirm('${accountId}'"]`
      );
      if (toggle) {
        toggle.checked = !enabled;
      }
...
function updateAutoConfirmRowStatus(accountId, enabled) {
  const row = document.querySelector(
    `tr:has(input[onchange*="toggleAutoConfirm('${accountId}'"])`
  );
  if (!row) return;

  const statusBadge = row.querySelector(
    '.status-badge:has(i.bi-truck, i.bi-truck-flatbed)'
  );
...
async function checkAuth() {
  ...
  try {
    ...
    if (result.is_admin === true) {
      ...
      // 显示注册设置功能
      const registrationSettings = document.getElementById(
...
async function openDefaultReplyManager() {
  try {
    ...
    const modal = new bootstrap.Modal(
      document.getElementById('defaultReplyModal')
    );
    modal.show();
  } catch (error) {
...
async function editDefaultReply(accountId) {
  try {
    ...
    document.getElementById('editDefaultReplyEnabled').checked =
      settings.enabled;
    document.getElementById('editReplyContent').value =
      settings.reply_content || '';
    document.getElementById('editReplyOnce').checked =
      settings.reply_once || false;
    ...
    // 显示编辑模态框
    const modal = new bootstrap.Modal(
      document.getElementById('editDefaultReplyModal')
    );
    modal.show();
  } catch (error) {
...
async function clearDefaultReplyRecords(accountId) {
  ...
  try {
    const response = await fetch(
      `${apiBase}/default-replies/${accountId}/clear-records`,
      {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${authToken}`,
...
async function configAIReply(accountId) {
  try {
    ...
    const settings = await fetchJSON(
      `${apiBase}/ai-reply-settings/${accountId}`
    );
    ...
    // 显示模态框
    const modal = new bootstrap.Modal(
      document.getElementById('aiReplyConfigModal')
    );
    modal.show();
  } catch (error) {
...
async function saveAIReplyConfig() {
  try {
    ...
    if (enabled) {
      ...
      // 验证自定义提示词格式
      const customPrompts = document
        .getElementById('customPrompts')
        .value.trim();
      if (customPrompts) {
        try {
    ...
    if (modelName === 'custom') {
      const customModelName = document
        .getElementById('customModelName')
        .value.trim();
      if (!customModelName) {
    ...
    const settings = {
      ...
      max_discount_percent: parseInt(
        document.getElementById('maxDiscountPercent').value
      ),
      max_discount_amount: parseInt(
        document.getElementById('maxDiscountAmount').value
      ),
      max_bargain_rounds: parseInt(
        document.getElementById('maxBargainRounds').value
      ),
...
async function testAIReply() {
  try {
    ...
    testReplyContent.innerHTML =
      '<i class="bi bi-hourglass-split"></i> 正在生成AI回复...';
...
const channelTypeConfigs = {
  qq: {
    title: 'QQ通知',
    description:
      '需要添加QQ号 <code>3668943488</code> 为好友才能正常接收消息通知',
...
function showAddChannelModal(type) {
  ...
  ).textContent = `添加${config.title}`;
  document.getElementById('channelTypeDescription').innerHTML =
    config.description;
...
function generateFieldHtml(field, prefix) {
  ...
  switch (field.type) {
    case 'select':
      ...
      break;
    case 'textarea':
      inputHtml = `<textarea class="form-control" id="${fieldId}" placeholder="${
        field.placeholder
    ...
    default:
      inputHtml = `<input type="${
        field.type
      }" class="form-control" id="${fieldId}" placeholder="${
        field.placeholder
      }" ${field.required ? 'required' : ''}>`;
  }
  ...
  channels.forEach((channel) => {
    ...
    tr.innerHTML = `
        ...
        <div class="d-flex align-items-center">
            <i class="bi ${
              typeConfig ? typeConfig.icon : 'bi-bell'
            } me-2 text-${typeColor}"></i>
            ${channel.name}
        </div>
        </td>
        ...
        <td>
        <div class="btn-group" role="group">
            <button class="btn btn-sm btn-outline-primary" onclick="editNotificationChannel(${
              channel.id
            })" title="编辑">
            <i class="bi bi-pencil"></i>
            </button>
            <button class="btn btn-sm btn-outline-danger" onclick="deleteNotificationChannel(${
              channel.id
            })" title="删除">
            <i class="bi bi-trash"></i>
            </button>
...
async function deleteNotificationChannel(channelId) {
  ...
  try {
    const response = await fetch(
      `${apiBase}/notification-channels/${channelId}`,
      {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${authToken}`,
...
function renderMessageNotifications(accounts, notifications) {
  ...
  accounts.forEach((accountId) => {
    ...
    if (accountNotifications.length > 0) {
      channelsList = accountNotifications
        .map(
          (n) =>
            `<span class="badge bg-${
              n.enabled ? 'success' : 'secondary'
            } me-1">${n.channel_name}</span>`
        )
        .join('');
    } else {
...
async function configAccountNotification(accountId) {
  try {
    ...
    const notificationResponse = await fetch(
      `${apiBase}/message-notifications/${accountId}`,
      {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
    ...
    const currentNotification =
      currentNotifications.length > 0 ? currentNotifications[0] : null;
    ...
    channels.forEach((channel) => {
      if (channel.enabled) {
        ...
        if (
          currentNotification &&
          currentNotification.channel_id === channel.id
        ) {
    ...
    // 设置启用状态
    document.getElementById('notificationEnabled').checked = currentNotification
      ? currentNotification.enabled
      : true;

    // 显示配置模态框
    const modal = new bootstrap.Modal(
      document.getElementById('configNotificationModal')
    );
    modal.show();
  } catch (error) {
...
async function deleteAccountNotification(accountId) {
  ...
  try {
    const response = await fetch(
      `${apiBase}/message-notifications/account/${accountId}`,
      {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${authToken}`,
...
function renderCardsList(cards) {
  ...
  cards.forEach((card) => {
    ...
    tr.innerHTML = `
        ...
        <td>
        <div class="btn-group" role="group">
            <button class="btn btn-sm btn-outline-primary" onclick="editCard(${
              card.id
            })" title="编辑">
            <i class="bi bi-pencil"></i>
            </button>
            <button class="btn btn-sm btn-outline-info" onclick="testCard(${
              card.id
            })" title="测试">
            <i class="bi bi-play"></i>
            </button>
...
function toggleMultiSpecFields() {
  ...
  document.getElementById('multiSpecFields').style.display = isMultiSpec
    ? 'block'
    : 'none';
}
...
function validateCardImageDimensions(file, inputElement) {
  ...
  img.onload = function () {
    ...
    if (width > maxDimension || height > maxDimension) {
      showToast(
        `❌ 图片尺寸过大：${width}x${height}，最大允许：${maxDimension}x${maxDimension}像素`,
        'warning'
      );
      inputElement.value = '';
      hideCardImagePreview();
    ...
    if (totalPixels > maxPixels) {
      showToast(
        `❌ 图片像素总数过大：${(totalPixels / 1024 / 1024).toFixed(
          1
        )}M像素，最大允许：8M像素`,
        'warning'
      );
    ...
    if (width > 2048 || height > 2048) {
      showToast(
        `ℹ️ 图片尺寸较大（${width}x${height}），上传时将自动压缩以优化性能`,
        'info'
      );
    } else {
...
function validateEditCardImageDimensions(file, inputElement) {
  ...
  img.onload = function () {
    ...
    if (width > 4096 || height > 4096) {
      showToast(
        `❌ 图片尺寸过大（${width}x${height}），最大支持 4096x4096 像素`,
        'warning'
      );
      inputElement.value = '';
      hideEditCardImagePreview();
...
function toggleEditMultiSpecFields() {
  ...
  // 验证设置是否生效
  console.log(
    'toggleEditMultiSpecFields - 实际显示样式:',
...
async function saveCard() {
  try {
    ...
    const cardData = {
      ...
      delay_seconds:
        parseInt(document.getElementById('cardDelaySeconds').value) || 0,
      enabled: true,
    ...
    switch (cardType) {
      case 'api':
        ...
        try {
          const headersInput = document
            .getElementById('apiHeaders')
            .value.trim();
          if (headersInput) {
...
function renderDeliveryRulesList(rules) {
  ...
  rules.forEach((rule) => {
    ...
    tr.innerHTML = `
        ...
        <td>
        <div class="btn-group" role="group">
            <button class="btn btn-sm btn-outline-primary" onclick="editDeliveryRule(${
              rule.id
            })" title="编辑">
            <i class="bi bi-pencil"></i>
            </button>
            <button class="btn btn-sm btn-outline-info" onclick="testDeliveryRule(${
              rule.id
            })" title="测试">
            <i class="bi bi-play"></i>
            </button>
            <button class="btn btn-sm btn-outline-danger" onclick="deleteDeliveryRule(${
              rule.id
            })" title="删除">
            <i class="bi bi-trash"></i>
            </button>
...
function updateDeliveryStats(rules) {
  ...
  const totalDeliveries = rules.reduce(
    (sum, rule) => sum + (rule.delivery_times || 0),
    0
  );
...
function showAddDeliveryRuleModal() {
  ...
  const modal = new bootstrap.Modal(
    document.getElementById('addDeliveryRuleModal')
  );
  modal.show();
}
  ...
  try {
    ...
    if (response.ok) {
      ...
      document.getElementById('editCardDescription').value =
        card.description || '';
      document.getElementById('editCardDelaySeconds').value =
        card.delay_seconds || 0;
      ...
      if (card.type === 'api' && card.api_config) {
        ...
        document.getElementById('editApiMethod').value =
          card.api_config.method || 'GET';
        document.getElementById('editApiTimeout').value =
          card.api_config.timeout || 10;
        document.getElementById('editApiHeaders').value =
          card.api_config.headers || '{}';
        document.getElementById('editApiParams').value =
          card.api_config.params || '{}';
      } else if (card.type === 'text') {
        document.getElementById('editTextContent').value =
          card.text_content || '';
      } else if (card.type === 'data') {
        document.getElementById('editDataContent').value =
          card.data_content || '';
      } else if (card.type === 'image') {
        // 处理图片类型
      ...
      setTimeout(() => {
        ...
        console.log(
          '多规格字段显示状态:',
          multiSpecElement
            ? multiSpecElement.style.display
            : 'element not found'
        );
      }, 100);
...
function toggleEditCardTypeFields() {
  ...
  document.getElementById('editApiFields').style.display =
    cardType === 'api' ? 'block' : 'none';
  document.getElementById('editTextFields').style.display =
    cardType === 'text' ? 'block' : 'none';
  document.getElementById('editDataFields').style.display =
    cardType === 'data' ? 'block' : 'none';
  document.getElementById('editImageFields').style.display =
    cardType === 'image' ? 'block' : 'none';
  ...
  if (cardType === 'api') {
    ...
    if (editApiMethodSelect) {
      editApiMethodSelect.removeEventListener(
        'change',
        toggleEditApiParamsHelp
      );
...
async function updateCard() {
  try {
    ...
    const cardData = {
      ...
      delay_seconds:
        parseInt(document.getElementById('editCardDelaySeconds').value) || 0,
    ...
    switch (cardType) {
      case 'api':
        ...
        try {
          const paramsInput = document
            .getElementById('editApiParams')
            .value.trim();
          if (paramsInput) {
        ...
        break;
      case 'text':
        cardData.text_content =
          document.getElementById('editTextContent').value;
        break;
      case 'data':
        cardData.data_content =
          document.getElementById('editDataContent').value;
        break;
      case 'image':
...
async function editDeliveryRule(ruleId) {
  try {
    ...
    if (response.ok) {
      ...
      document.getElementById('editDeliveryCount').value =
        rule.delivery_count || 1;
      ...
      // 显示模态框
      const modal = new bootstrap.Modal(
        document.getElementById('editDeliveryRuleModal')
      );
      modal.show();
    } else {
...
async function updateDeliveryRule() {
  try {
    ...
    const deliveryCount =
      document.getElementById('editDeliveryCount').value || 1;
...
async function uploadDatabaseBackup() {
  ...
  if (
    !confirm(
      '恢复数据库将完全替换当前所有数据，包括所有用户、Cookie、卡券等信息。\n\n此操作不可撤销！\n\n确定要继续吗？'
    )
  ) {
    return;
  }
...
async function toggleItemMultiSpec(cookieId, itemId, isMultiSpec) {
  try {
    const response = await fetch(
      `${apiBase}/items/${encodeURIComponent(cookieId)}/${encodeURIComponent(
        itemId
      )}/multi-spec`,
      {
        method: 'PUT',
...
// 切换商品多数量发货状态
async function toggleItemMultiQuantityDelivery(
  cookieId,
  itemId,
  multiQuantityDelivery
) {
  try {
    const response = await fetch(
      `${apiBase}/items/${encodeURIComponent(cookieId)}/${encodeURIComponent(
        itemId
      )}/multi-quantity-delivery`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (response.ok) {
      showToast(
        `${multiQuantityDelivery ? '开启' : '关闭'}多数量发货成功`,
        'success'
      );
      // 刷新商品列表
      await refreshItemsData();
...
async function loadItemsByCookie() {
  ...
  try {
    const response = await fetch(
      `${apiBase}/items/cookie/${encodeURIComponent(cookieId)}`,
      {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
...
function displayCurrentPageItems() {
  ...
  if (!filteredItemsData || filteredItemsData.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="10" class="text-center text-muted">暂无商品数据</td></tr>';
    resetItemsSelection();
    return;
  }
  ...
  const itemsHtml = currentPageItems
    .map((item) => {
      // 处理商品标题显示
      ...
      if (item.item_detail) {
        ...
        itemDetailDisplay =
          detailText.substring(0, 50) + (detailText.length > 50 ? '...' : '');
      }
      ...
      return `
            ...
            <td title="${escapeHtml(item.item_title || '未设置')}">${escapeHtml(
        itemTitleDisplay
      )}</td>
            <td title="${escapeHtml(
              getItemDetailText(item.item_detail || '')
            )}">${escapeHtml(itemDetailDisplay)}</td>
            ...
            <td>
                <div class="btn-group" role="group">
                <button class="btn btn-sm btn-outline-primary" onclick="editItem('${escapeHtml(
                  item.cookie_id
                )}', '${escapeHtml(item.item_id)}')" title="编辑详情">
                    <i class="bi bi-pencil"></i>
                </button>
                <button class="btn btn-sm btn-outline-danger" onclick="deleteItem('${escapeHtml(
                  item.cookie_id
                )}', '${escapeHtml(item.item_id)}', '${escapeHtml(
        item.item_title || item.item_id
      )}')" title="删除">
                    <i class="bi bi-trash"></i>
                </button>
                <button class="btn btn-sm ${
                  isMultiSpec ? 'btn-warning' : 'btn-success'
                }" onclick="toggleItemMultiSpec('${escapeHtml(
        item.cookie_id
      )}', '${escapeHtml(item.item_id)}', ${!isMultiSpec})" title="${
        isMultiSpec ? '关闭多规格' : '开启多规格'
      }">
                    <i class="bi ${
                      isMultiSpec ? 'bi-toggle-on' : 'bi-toggle-off'
                    }"></i>
                </button>
                <button class="btn btn-sm ${
                  isMultiQuantityDelivery ? 'btn-warning' : 'btn-success'
                }" onclick="toggleItemMultiQuantityDelivery('${escapeHtml(
        item.cookie_id
      )}', '${escapeHtml(
        item.item_id
      )}', ${!isMultiQuantityDelivery})" title="${
        isMultiQuantityDelivery ? '关闭多数量发货' : '开启多数量发货'
      }">
                    <i class="bi ${
                      isMultiQuantityDelivery
                        ? 'bi-box-arrow-down'
                        : 'bi-box-arrow-up'
                    }"></i>
                </button>
...
async function getAllItemsFromAccount() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.success) {
        showToast(
          `成功获取第${pageNumber}页 ${data.current_count} 个商品，请查看控制台日志`,
          'success'
        );
        // 刷新商品列表（保持筛选器选择）
        await refreshItemsData();
...
async function batchDeleteItems() {
  try {
    ...
    // 确认删除
    const confirmed = confirm(
      `确定要删除选中的 ${checkboxes.length} 个商品信息吗？\n\n此操作不可撤销！`
    );
    if (!confirmed) {
      return;
    }
    ...
    if (response.ok) {
      const result = await response.json();
      showToast(
        `批量删除完成: 成功 ${result.success_count} 个，失败 ${result.failed_count} 个`,
        'success'
      );
      // 刷新列表（保持筛选器选择）
      await refreshItemsData();
...
async function loadItemsReplayByCookie() {
  ...
  try {
    const response = await fetch(
      `${apiBase}/itemReplays/cookie/${encodeURIComponent(cookieId)}`,
      {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
...
function displayItemReplays(items) {
  ...
  if (!items || items.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="5" class="text-center text-muted">暂无商品数据</td></tr>';
    // 重置选择状态
    const selectAllCheckbox = document.getElementById('selectAllItems');
    if (selectAllCheckbox) {
      selectAllCheckbox.checked = false;
  ...
  const itemsHtml = items
    .map((item) => {
      ...
      if (item.item_detail) {
        try {
          ...
          if (detail.content) {
            ...
            itemDetailDisplay =
              item.item_detail.substring(0, 50) +
              (item.item_detail.length > 50 ? '...' : '');
          }
        } catch (e) {
      ...
      return `
        ...
        <td title="${escapeHtml(item.item_detail || '未设置')}">${escapeHtml(
        itemDetailDisplay
      )}</td>
        <td title="${escapeHtml(item.reply_content || '未设置')}">${escapeHtml(
        item.reply_content
      )}</td>
        <td>${formatDateTime(item.updated_at)}</td>
        <td>
            <div class="btn-group" role="group">
            <button class="btn btn-sm btn-outline-primary" onclick="editItemReply('${escapeHtml(
              item.cookie_id
            )}', '${escapeHtml(item.item_id)}')" title="编辑详情">
                <i class="bi bi-pencil"></i>
            </button>
            <button class="btn btn-sm btn-outline-danger" onclick="deleteItemReply('${escapeHtml(
              item.cookie_id
            )}', '${escapeHtml(item.item_id)}', '${escapeHtml(
        item.item_title || item.item_id
      )}')" title="删除">
...
async function showItemReplayEdit() {
  // 显示模态框
  const modal = new bootstrap.Modal(
    document.getElementById('editItemReplyModal')
  );
...
async function editItemReply(cookieId, itemId) {
  try {
    const response = await fetch(
      `${apiBase}/item-reply/${encodeURIComponent(
        cookieId
...
async function saveItemReply() {
  ...
  const replyContent = document
    .getElementById('editItemReplyContent')
    .value.trim();
...
async function batchDeleteItemReplies() {
  try {
    ...
    if (response.ok) {
      const result = await response.json();
      showToast(
        `批量删除回复完成: 成功 ${result.success_count} 个，失败 ${result.failed_count} 个`,
        'success'
      );
      await loadItemsReplayByCookie?.();
    } else {
...
function displayLogs() {
  ...
  const logsHtml = window.filteredLogs
    .map((log) => {
      ...
      if (data.success) {
        ...
        let statsHtml = `
            ...
            <div class="col-md-6">
                ...
                <li>使用率: <strong>${(
                  (stats.total_logs / stats.max_capacity) *
                  100
                ).toFixed(1)}%</strong></li>
                </ul>
            </div>
        ...
        sources.forEach(([source, count], index) => {
          if (index % 2 === 0)
            statsHtml += '<div class="col-md-6"><ul class="list-unstyled">';
          const percentage = ((count / stats.total_logs) * 100).toFixed(1);
          statsHtml += `<li>${source}: <strong>${count}</strong> (${percentage}%)</li>`;
          if (index % 2 === 1 || index === sources.length - 1)
            statsHtml += '</ul></div>';
        });
        ...
        // 显示模态框
        const modal = new bootstrap.Modal(
          document.getElementById('logStatsModal')
        );
        modal.show();
      } else {
...
async function exportKeywords() {
  ...
  try {
    const response = await fetch(
      `${apiBase}/keywords-export/${currentCookieId}`,
      {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
...
function showImportModal() {
  ...
  const modal = new bootstrap.Modal(
    document.getElementById('importKeywordsModal')
  );
  modal.show();
}
...
async function importKeywords() {
  ...
  try {
    ...
    const response = await fetch(
      `${apiBase}/keywords-import/${currentCookieId}`,
      {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${authToken}`,
    ...
    if (response.ok) {
      ...
      setTimeout(() => {
        ...
        showToast(
          `导入成功！新增: ${result.added}, 更新: ${result.updated}`,
          'success'
        );
      }, 500);
    } else {
...
function showQRCodeLogin() {
  const modal = new bootstrap.Modal(
    document.getElementById('qrCodeLoginModal')
  );
  modal.show();
...
async function checkQRCodeStatus() {
  ...
  try {
    const response = await fetch(
      `${apiBase}/qr-login/check/${qrCodeSessionId}`,
      {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
...
function showVerificationRequired(data) {
  if (data.verification_url) {
    ...
    if (!verificationContainer) {
      ...
      document
        .querySelector('#qrCodeLoginModal .modal-body')
        .appendChild(verificationContainer);
    }
...
function showAddImageKeywordModal() {
  ...
  // 显示模态框
  const modal = new bootstrap.Modal(
    document.getElementById('addImageKeywordModal')
  );
  modal.show();
...
async function addImageKeyword() {
  ...
  try {
    ...
    const response = await fetch(
      `${apiBase}/keywords/${currentCookieId}/image`,
      {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${authToken}`,
...
async function exportKeywords() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // 从响应头获取文件名
      const contentDisposition = response.headers.get('Content-Disposition');
      let fileName = `关键词数据_${currentCookieId}_${new Date()
        .toISOString()
        .slice(0, 10)}.xlsx`;
...
function editRemark(cookieId, currentRemark) {
  ...
  const remarkCell = document.querySelector(
    `[data-cookie-id="${cookieId}"] .remark-display`
  );
  if (!remarkCell) {
  ...
  const saveRemark = async () => {
    console.log(
      'saveRemark called, isProcessing:',
      isProcessing,
      'hasChanged:',
      hasChanged
    ); // 调试信息
    ...
    try {
      ...
      if (response.ok) {
        // 更新显示
        remarkCell.innerHTML = `
                    <span class="remark-display" onclick="editRemark('${cookieId}', '${newRemark.replace(
          /'/g,
          '&#39;'
        )}')" title="点击编辑备注" style="cursor: pointer; color: #6c757d; font-size: 0.875rem;">
                        ${
...
function editPauseDuration(cookieId, currentDuration) {
  ...
  const pauseCell = document.querySelector(
    `[data-cookie-id="${cookieId}"] .pause-duration-display`
  );
  if (!pauseCell) {
  ...
  const savePauseDuration = async () => {
    console.log(
      'savePauseDuration called, isProcessing:',
      isProcessing,
      'hasChanged:',
      hasChanged
    ); // 调试信息
    ...
    try {
      const response = await fetch(
        `${apiBase}/cookies/${cookieId}/pause-duration`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        showToast(
          `暂停时间更新失败: ${errorData.detail || '未知错误'}`,
          'danger'
        );
        // 恢复原始内容
        pauseCell.innerHTML = originalContent;
...
async function loadSystemSettings() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // 显示/隐藏管理员专用设置（仅管理员可见）
      const apiSecuritySettings = document.getElementById(
...
async function updateQQReplySecretKey() {
  const qqReplySecretKey = document
    .getElementById('qqReplySecretKey')
    .value.trim();
...
function generateOutgoingFieldHtml(field, value) {
  switch (field.type) {
    case 'select':
      ...
      return `<select class="form-select" id="${field.id}" name="${field.id}" ${
        field.required ? 'required' : ''
      }>${options}</select>`;

    case 'password':
      return `<input type="password" class="form-control" id="${
        field.id
      }" name="${field.id}" value="${value}" placeholder="${
        field.placeholder
      }" ${field.required ? 'required' : ''}>`;
    ...
    case 'email':
      return `<input type="email" class="form-control" id="${field.id}" name="${
        field.id
    ...
    default:
      return `<input type="text" class="form-control" id="${field.id}" name="${
        field.id
      }" value="${value}" placeholder="${field.placeholder}" ${
        field.required ? 'required' : ''
      }>`;
...
async function loadOrderCookieFilter() {
  try {
    ...
    if (data.success && data.data) {
      ...
      const cookieIds = [
        ...new Set(
          data.data.map((order) => order.cookie_id).filter((id) => id)
        ),
      ];
...
async function loadOrdersByCookie() {
  ...
  try {
    ...
    if (data.success) {
      // 筛选指定Cookie的订单
      allOrdersData = (data.data || []).filter(
        (order) => order.cookie_id === selectedCookie
      );
      // 按创建时间倒序排列
      allOrdersData.sort(
        (a, b) => new Date(b.created_at) - new Date(a.created_at)
...
function filterOrders() {
  ...
  filteredOrdersData = allOrdersData.filter((order) => {
    ...
    const matchesSearch =
      !searchKeyword ||
      (order.order_id &&
        order.order_id.toLowerCase().includes(searchKeyword)) ||
      (order.item_id && order.item_id.toLowerCase().includes(searchKeyword)) ||
      (order.buyer_id &&
        order.buyer_id.toLowerCase().includes(searchKeyword)) ||
      (order.buyer_nickName &&
        order.buyer_nickName.toLowerCase().includes(searchKeyword));
...
function createOrderRow(order) {
  ...
  return `
        <tr>
            ...
            <td>
                <span class="text-truncate d-inline-block" style="max-width: 120px;" title="${
                  order.order_id
                }">
                    ${order.order_id}
                </span>
            ...
            <td>
                <span class="text-truncate d-inline-block" style="max-width: 100px;" title="${
                  order.item_id || ''
                }">
                    ${order.item_id || '-'}
                </span>
            ...
            <td>
                <span class="text-truncate d-inline-block" style="max-width: 80px;" title="${
                  order.buyer_id || ''
                }">
                    ${order.buyer_id || '-'}
                </span>
            ...
            <td>
                <div class="btn-group btn-group-sm" role="group">
                    ...
                    <button class="btn btn-outline-danger btn-sm" onclick="deleteOrder('${
                      order.order_id
                    }')" title="删除">
                        <i class="bi bi-trash"></i>
                    </button>
...
function updateOrdersPagination() {
  ...
  if (nextPageBtn)
    nextPageBtn.disabled =
      currentOrdersPage === totalOrdersPages || totalOrdersPages === 0;
  if (lastPageBtn)
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <table class="table table-sm">
                                        ...
                                        <tr><td>买家昵称</td><td>${
                                          order.buyer_nickName || '未知'
                                        }</td></tr>
                                        <tr><td>买家姓名</td><td>${
                                          order.buyer_name || '未知'
                                        }</td></tr>
                                        <tr><td>买家电话</td><td>${
                                          order.buyer_phone || '未知'
                                        }</td></tr>
                                        <tr><td>买家地址</td><td>${
                                          order.buyer_address || '未知'
                                        }</td></tr>
                                        <tr><td>Cookie账号</td><td>${
                                          order.cookie_id || '未知'
                                        }</td></tr>
                                        <tr><td>订单状态</td><td><span class="badge ${getOrderStatusClass(
                                          order.order_status
                                        )}">${getOrderStatusText(
      order.order_status
    )}</span></td></tr>
                            ...
                            <div class="row mt-3">
                                <div class="col-12">
                                    <h6>时间信息</h6>
                                    <table class="table table-sm">
                                        <tr><td>创建时间</td><td>${formatDateTime(
                                          order.created_at
                                        )}</td></tr>
                                        <tr><td>更新时间</td><td>${formatDateTime(
                                          order.updated_at
                                        )}</td></tr>
                                    </table>
                                </div>
      ...
      content.innerHTML = `
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <small class="text-muted">分类：${
                                  item.item_category || '未知'
                                }</small>
                            </div>
                            <div class="col-md-6">
                                <small class="text-muted">价格：${
                                  item.item_price || '未知'
                                }</small>
                            </div>
                        </div>
...
async function loadUserSystemStats() {
  try {
    ...
    if (cookiesResponse.ok) {
      const cookiesData = await cookiesResponse.json();
      document.getElementById('totalUserCookies').textContent = cookiesData.data
        ? cookiesData.data.length
        : 0;
    }
    ...
    if (cardsResponse.ok) {
      ...
      document.getElementById('totalUserCards').textContent = cardsData.data
  ...
  col.innerHTML = `
        <div class="card user-card h-100">
            <div class="card-body">
                ...
                <p class="card-text text-muted small">
                    <i class="bi bi-calendar me-1"></i>注册时间：${formatDateTime(
                      user.created_at
                    )}
                </p>
      ...
      const dataCells = columns
        .map((col) => {
          ...
          if (value === null || value === undefined) {
            ...
            value = `<span title="${escapeHtml(value)}">${escapeHtml(
...
function updateTableInfo(tableName, recordCount) {
  ...
  document.getElementById(
    'tableTitle'
  ).innerHTML = `<i class="bi bi-table"></i> ${description}`;
...
async function exportTableData() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      a.style.display = 'none';
      a.href = url;
      a.download = `${currentTable}_${new Date()
        .toISOString()
        .slice(0, 10)}.xlsx`;
      document.body.appendChild(a);
      a.click();
...
function initDeleteRecordModal() {
  if (!deleteRecordModal) {
    deleteRecordModal = new bootstrap.Modal(
...
function updateLogInfo(data) {
  ...
  document.getElementById('logDisplayLines').textContent =
    data.total_lines || '-';
}
...
function displayRiskControlLogs(logs) {
  ...
  logs.forEach((log) => {
    ...
    row.innerHTML = `
            ...
            )}">${escapeHtml(log.event_description || '-')}</td>
            <td class="text-truncate" style="max-width: 200px;" title="${escapeHtml(
              log.processing_result || '-'
            )}">${escapeHtml(log.processing_result || '-')}</td>
            <td>
                <button class="btn btn-sm btn-outline-danger" onclick="deleteRiskControlLog(${
                  log.id
                })" title="删除">
                    <i class="bi bi-trash"></i>
                </button>
...
function updateRiskLogInfo(data) {
  ...
  if (paginationInfo) {
    ...
    const end = Math.min(
      currentRiskLogOffset + (data.data ? data.data.length : 0),
      data.total || 0
    );
    paginationInfo.textContent = `显示第 ${start}-${end} 条，共 ${
      data.total || 0
    } 条记录`;
  }
}
...
function updateRiskLogPagination(data) {
  ...
  prevLi.innerHTML = `<a class="page-link" href="#" onclick="loadRiskControlLogs(${
    (currentPage - 2) * limit
  })">上一页</a>`;
  ...
  for (let i = startPage; i <= endPage; i++) {
    ...
    li.innerHTML = `<a class="page-link" href="#" onclick="loadRiskControlLogs(${
      (i - 1) * limit
    })">${i}</a>`;
  ...
  nextLi.innerHTML = `<a class="page-link" href="#" onclick="loadRiskControlLogs(${
    currentPage * limit
  })">下一页</a>`;
...
function filterRiskLogsByStatus(status) {
  ...
  document
    .querySelector(`.filter-badge[data-status="${status}"]`)
    ...
    if (response.ok) {
      ...
      if (data.success && data.cookies) {
        data.cookies.forEach((cookie) => {
          ...
          option.textContent = `${cookie.cookie_id} (${
            cookie.nickname || '未知'
          })`;
          select.appendChild(option);
        });
...
async function handleItemSearch(event) {
  ...
  const totalPages =
    parseInt(document.getElementById('searchTotalPages').value) || 1;
  const pageSize =
    parseInt(document.getElementById('searchPageSize').value) || 20;
  ...
  try {
    ...
    if (cookiesCheckResponse.ok) {
      ...
      if (!cookiesData.hasValidCookies) {
        showToast(
          '搜索失败：系统中不存在有效的账户信息。请先在Cookie管理中添加有效的闲鱼账户。',
          'warning'
        );
...
function createItemCard(item) {
  ...
  const imageUrl =
    item.main_image ||
    item.image_url ||
    'https://via.placeholder.com/200x200?text=图片加载失败';
  ...
  col.innerHTML = `
        <div class="card item-card h-100">
            <img src="${escapeHtml(
              imageUrl
            )}" class="item-image" alt="${escapeHtml(item.title)}"
...
function updateSearchStats(data) {
  document.getElementById('totalItemsFound').textContent =
    searchResultsData.length;
  document.getElementById('totalPagesSearched').textContent =
    data.total_pages || 0;
...
function updateSearchPagination() {
  ...
  pagination.innerHTML = `
        <ul class="pagination">
            ...
            <li class="page-item ${
              currentSearchPage === totalSearchPages ? 'disabled' : ''
            }">
                <a class="page-link" href="#" onclick="changeSearchPage(${
                  currentSearchPage + 1
                })">下一页</a>
...
function generateSearchPageNumbers() {
  ...
  let startPage = Math.max(
    1,
    currentSearchPage - Math.floor(maxVisiblePages / 2)
  );
...
function changeSearchPage(page) {
  ...
  displaySearchResults();
  updateSearchStats({
    total_pages: document.getElementById('totalPagesSearched').textContent,
  });
}
...
function exportSearchResults() {
  ...
  try {
    ...
    link.setAttribute(
      'download',
      `商品搜索结果_${new Date().toISOString().slice(0, 10)}.csv`
    );
...
async function showProjectStats() {
  try {
    ...
    // 显示模态框
    const modal = new bootstrap.Modal(
      document.getElementById('projectStatsModal')
    );
    modal.show();

    // 模态框关闭后移除DOM元素
    document
      .getElementById('projectStatsModal')
      .addEventListener('hidden.bs.modal', function () {
        this.remove();
      });
  } catch (error) {
...
async function loadSystemVersion() {
  try {
    ...
    // 获取远程版本并检查更新
    const response = await fetch(
      'http://xianyu.zhinianblog.cn/index.php?action=getVersion'
    );
    const result = await response.json();
...
async function getUpdateInfo() {
  try {
    const response = await fetch(
      'http://xianyu.zhinianblog.cn/index.php?action=getUpdateInfo'
    );
    const result = await response.json();
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        ${
                          updateList
                            ? `<ul class="list-unstyled ps-3">${updateList}</ul>`
                            : '<p class="text-muted">暂无更新内容</p>'
                        }
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">关闭</button>
